...
public struct DefaultSummary {
    public static func balanceSections(from store: AccountStore, materiality: Decimal = 0) -> StatementDef {
        ...
        let assetOms = Set(store.all.filter { $0.direction == .debit }.compactMap { $0.identifiers.omslag })
        let liabEqOms = Set(store.all.filter { $0.direction == .credit }.compactMap { $0.identifiers.omslag })
    ...
    public static func incomeSections(from store: AccountStore, materiality: Decimal = 0) -> StatementDef {
        ...
        let revenueRGS = Set(store.all.filter { $0.direction == .credit }.map { $0.identifiers.rgs.isEmpty ? $0.code : $0.identifiers.rgs })
        let expenseRGS = Set(store.all.filter { $0.direction == .debit  }.map { $0.identifiers.rgs.isEmpty ? $0.code : $0.identifiers.rgs })
