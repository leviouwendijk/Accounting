...
public enum AssetsOverviewBuilder {
    public static func build(
        ...
        for (key, entity) in entities.pairs() {
            ...
            let acquisitionDate = entity.profile?.acquisitionDate
            ...
            let availableFrom = commissionDate ?? acquisitionDate

            if let commissionDate, commissionDate > closingAsOf {
                continue
            }
            ...
            if acquisitionDate == nil && commissionDate == nil {
            ...
            let periodInvestment = investmentAmount(
                acquisitionCost: acquisitionCost,
                investmentDate: commissionDate ?? acquisitionDate,
                in: period
            )
    ...
    private static func extractAcquisitionCost(
        ...
        if let cost = entity.profile?.acquisitionCost?.cost,
           cost > 0
        {
            return cost
        }
    ...
    private static func groupLabel(
        ...
        if let explicit = cleaned(entity.metadata["asset_group"])
            ?? cleaned(entity.metadata["group"])
            ?? cleaned(entity.metadata["asset_category"])
            ?? cleaned(entity.metadata["category"])
            ?? cleaned(entity.metadata["subtype"])
        {
            return humanize(explicit)
        }

        let keyString = key.identifier(displaying: .fullchain).lowercased()

        if keyString.contains("vehicle")
            || keyString.contains("auto")
            || keyString.contains("car")
        {
            return "Vehicles"
        }

        if keyString.contains("computer")
            || keyString.contains("laptop")
            || keyString.contains("macbook")
            || keyString.contains("equipment")
            || keyString.contains("gear")
        {
            return "Equipment"
        }

        return humanize(key.family)
    }
