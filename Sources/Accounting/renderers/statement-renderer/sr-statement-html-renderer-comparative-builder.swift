...
extension StatementHTMLRenderer {
    ...
    static func mergeComparativeSection(
        ...
        let hierarchy = RenderedRowHierarchyBuilder.makeMap(
            idsInOrder: mergedBaseRows.compactMap(\.id),
            parentById: maps.parentById
        )

        let mergedRows: [ComparativeRow] = mergedBaseRows.map { row in
            let h = row.id.flatMap { hierarchy[$0] }
            let depth = h?.depth ?? 0
            ...
            return ComparativeRow(
                ...
                label: row.label,
                cells: [
                    row.currentAmount.map { .value($0) } ?? .blank,
                    row.previousAmount.map { .value($0) } ?? .blank,
                ],
